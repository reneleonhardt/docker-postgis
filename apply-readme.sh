...
# Update README.md from the _dockerlists_*.md files generated by ./update.sh.
# Run separately only when README.md needs a manual update.
echo " "
...
if sed --version >/dev/null 2>&1; then
    ...
    sed -i '' -E "s/(## Versions) \([0-9]{4}-[0-9]{2}-[0-9]{2}\)/\1 ($TODAY)/g" README.md
fi
...
for readme_group in debian alpine test; do
    ...
    awk -v readme_group="$readme_group" -v content_file="_dockerlists_${readme_group}.md" '
    $0 ~ "<!-- "readme_group"_autogenerated_begin  -->" {
        print
...
if file_size=$(stat --format=%s "README.md" 2>/dev/null); then
    :
else
    file_size=$(wc -c < "README.md" | tr -d '[:space:]')
fi
echo " "
